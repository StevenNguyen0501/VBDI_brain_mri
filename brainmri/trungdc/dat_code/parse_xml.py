        ...
        if label.attrib['type'] == 'local':
            ...
            for class_name in class_names:
                for i in range(len(points_out)):
                    study_out.append([studyUid, seriesUid, points_out[i,-1], class_name, int(points_out[i,0]), int(points_out[i,1]), int(points_out[i,2]), int(points_out[i,3])])
    return study_out
...
if __name__ == "__main__":
    ...
    for study_i in tqdm(_1511_studies):
        try:
            ...
            df_i = pd.DataFrame(data=study_out, columns=["studyUid", "seriesUid", "imageUid", "label", "x1", "x2", "y1", "y2"])
            df = pd.concat([df, df_i])
        except:
            print(study_i)
    df.to_csv("brain-mri-dataset.csv", index=False)    