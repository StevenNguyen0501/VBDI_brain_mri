...
summary_dicom_path = "/home/single1/BACKUP/tintrung/brainmri/trungdc/summary_dicom.pkl"
summary_anot_path = "/home/single1/BACKUP/tintrung/brainmri/trungdc/summary_anot.pkl"
...
for seriesuid in seriesuids:
    ...
    cur = 0
    df_dicom_extract = df_dicom[df_dicom.SeriesInstanceUID == seriesuid].sort_values(by = ["SliceLocation"])
    df_anot_extract = df_anot[df_anot.seriesuid == seriesuid]
    try:
        out = series_vis(seriesuid, df_anot_extract, df_dicom_extract)
        total_df.extend(out)
    ...
    # if check == 3:
    #     break
total_df = pd.DataFrame(total_df, columns = ["SliceIndex", "Bbox", "Tag","LesionIndex","DicomFileName", "SeriesUID", "StudyUID"])
total_df.head()

pd.to_pickle(total_df, "bbox.pkl")

# %%


df = pd.read_pickle("bbox.pkl")
